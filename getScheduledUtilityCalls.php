...
if(isset($_POST['search_now'])){
     foreach($_POST as $name=>$value){
        ...
        $search_string = "SELECT  $utility_table.*, $account_table.division, $account_table.name,$account_table.city, $account_table.state, $account_table.zip,$account_table.address FROM $utility_table INNER JOIN $account_table  ON $utility_table.account_no = $account_table.account_ID  WHERE $utility_table.deleted =0 AND  route_status IN('scheduled','new') $addtn". $criteria1." ".$criteria2 ;
         echo $search_string;
         $check  = $db->query($search_string);
        
} else if( isset($_POST['scrs']) ){
    $search_string = "SELECT  $utility_table.*, $account_table.division, $account_table.name,$account_table.city, $account_table.state, $account_table.zip,$account_table.address FROM $utility_table INNER JOIN $account_table  ON $utility_table.account_no = $account_table.account_ID  WHERE $utility_table.deleted =0 AND  route_status IN('scheduled','new') $addtn" ;
    $check  = $db->query($search_string);
} else  {
    $search_string = "SELECT  $utility_table.*, $account_table.division, $account_table.name,$account_table.city, $account_table.state, $account_table.zip,$account_table.address FROM $utility_table INNER JOIN $account_table  ON $utility_table.account_no = $account_table.account_ID  WHERE $utility_table.deleted =0 AND  route_status IN('scheduled','new') $addtn" ;
    //echo $search_string;
