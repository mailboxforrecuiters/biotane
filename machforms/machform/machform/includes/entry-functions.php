							...
							if(!empty($options['strip_download_link'])){
								...
								if($show_image_preview === true && $is_image_file){
									if(!empty($options['machform_path'])){ //if the form is called from advanced form code
										...
										$entry_details[$i]['value'] .= '<img src="http'.$ssl_suffix.'://'.$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF']).'/view_img.php?q='.$q_string.'" style="max-width: 350px;margin: 15px 0;" /><br/><a class="entry_link" href="http'.$ssl_suffix.'://'.$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF']).'/download.php?q='.$q_string.'">'.$filename_value.'</a><br/>';
									}
								}else{
									if(!empty($options['machform_path'])){ //if the form is called from advanced form code
										$entry_details[$i]['value'] .= '<a class="entry_link" href="'.$options['machform_path'].'download.php?q='.$q_string.'">'.$filename_value.'</a><br/>';
									}else{
										$entry_details[$i]['value'] .= '<a class="entry_link" href="http'.$ssl_suffix.'://'.$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF']).'/download.php?q='.$q_string.'">'.$filename_value.'</a><br/>';
									}
								}
							}
