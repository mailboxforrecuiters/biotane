...
class Vehicle {
    ...
    function __construct($id= NULL){
        ...
        global $dbprefix;
        if(isset($id)){
            $result = $db->where("truck_id",$id)->get("assets.truck");   
            if(count($result)>0){
                $this->truck_id = $result[0]['truck_id'];//asset module truck id
                ...
                $this->typex = "truck";
                $this->module_id = $result[0]['module_id'];//individual module id
            }
        }
    }    
    
    function SelectListModule($x =NULL){
        $db = new Database();
        $select ="<select id='vehicle' name='vehicle'>";
        $ryt = $db->get("iwp_truck_id");
        if(count($ryt) !=0){
            foreach($ryt as $truck){
                ...
                $select .="<option $mark value='$truck[truck_id]'>$truck[name]</option>";
            }
        }    
        $select .="</select>";
    ...
    function SelectListAsset($x =NULL){
        $db = new Database();
        $select ="<select id='vehicle' name='vehicle'>";
        $ryt = $db->get("assets.truck");
        if(count($ryt) !=0){
            foreach($ryt as $truck){
                if($x == $truck['truck_id']){
                    $mark = " selected "; 
