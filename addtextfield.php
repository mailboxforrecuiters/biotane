...
if(count($oil_data)>0){// if stop has been picked up load data from oil table 
    ...
    foreach($oil_data as $info){
        ...
        echo  "<tr>
                    <td style='text-align:right'><input style='width:50px;' type='hidden' value='$info[entry_number]' class='entry' />Inches Collected </td>
                    <td  style='text-align:left'><input $read_only type='text' style='width: 50px;' value='$info[inches_entered]' placeholder='0.0' class='addtional_inch' formula='$gpi' total_cap='$info[expected_gallons]'/><input type='text' value='$info[inches_to_gallons]' style='width: 50px;' placeholder='0.0' class='aditional_galls' readonly=''/>/".$gpi_ask[0]['amount_holds']."<span style='font-size: 9px;color:#bbbbbb;'> Expected</span></td>
                    </tr>
                    <tr>";
                    
                    /*
                    echo "<td>Inches Leftover</td><td style='float:left;'>
    ...
    if(!empty($account->barrel_info)){
        foreach($account->barrel_info as $info){
            echo  "<tr>
                <td style='text-align:right'><input style='width:50px;' type='hidden' value='0' class='entry' readonly/>Inches Collected </td> 
                <td  style='text-align:left'> 
        ...
        echo  "<tr>
                ...
                <td  style='text-align:left'> 
                    <input disabled type='text' style='width: 50px;' value='' placeholder='0.0' class='addtional_inch' formula='0' total_cap='0'/> <input disabled type='text' value='0' style='width: 50px;' placeholder='0.0' class='aditional_galls' readonly=''/>  / 0<span style='font-size: 9px;color:#bbbbbb;'> Expected</span></td>
                    </tr>";
